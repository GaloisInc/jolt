...
const TEMPLATE_DIR: &str = "package-template";

fn main() {
    let manifest_dir =
        env::var_os("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR not set in environment");
    let template_dir = Path::new(&manifest_dir).join(TEMPLATE_DIR);
    if !template_dir.is_dir() {
        panic!("template directory {template_dir:?} is not a directory");
    }
    let template = read_fs_tree_recursively(&template_dir).unwrap();
    ...
    println!(
        "cargo::rustc-env=TEMPLATE_YAML_PATH={}",
        serialized_template_path.as_os_str().to_string_lossy()
    );
}
