    ...
    for DenseInterleavedPolynomial<F>
{
    ...
    fn compute_cubic(&self, eq_poly: &GruenSplitEqPolynomial<F>, previous_round_claim: F) -> UniPoly<F> {
        ...
        let quadratic_evals = if eq_poly.E_in_current_len() == 1 {
            ...
            eq_poly.E_out_current()
                ...
                .map(|(E_out_eval, P_x2)| {
                    ...
                    for (E_in_eval, P_chunk) in eq_poly.E_in_current().iter().zip(P_x2.chunks(4)) {
                        let left = (
        ...
        #[cfg(test)]
        {
            let eq_evals = crate::poly::eq_poly::EqPolynomial::evals(&eq_poly.w[..eq_poly.current_index]);
