...
impl<const C: usize, F: JoltField, I: ConstraintInput> CombinedUniformBuilder<C, F, I> {
    ...
    #[tracing::instrument(skip_all)]
    pub fn streaming_compute_spartan_Az_Bz_Cz<PCS: CommitmentScheme<Field = F>>(
        &self,
        flattened_polynomial_stream: impl Iterator<Item = Vec<F>>, // witness_value[step][r1cs_variable]
    ) -> (
        SparsePolynomial<F>,
        SparsePolynomial<F>,
        SparsePolynomial<F>,
        ...
        let uni_constraint_evals = flattened_polynomial_stream.enumerate().map(|local_index, item| {
            todo!()
        });
    ...
    pub fn compute_spartan_Az_Bz_Cz<PCS: CommitmentScheme<Field = F>>(
        &self,
        flattened_polynomials: &[&DensePolynomial<F>], // foreach r1cs_variable, foreach step, witness_value[r1cs_variable][step]
    ) -> (
        SparsePolynomial<F>,
        SparsePolynomial<F>,
        SparsePolynomial<F>,
